...
<template>
    <div>
        <h1 class="text-2xl font-medium dark:text-gray-300">Network Statistics</h1>
...
export default {
    ...
    methods: {
        ...
        earnings1() {
            this.axios.get("network/earnings/6").then((response) => {
                const apiResponse = response.data
                this.earnings6h = `${apiResponse.total_earnings}`
            })
        },
        earnings24() {
            this.axios.get("network/earnings/24").then((response) => {
                const apiResponse = response.data
                this.earnings24h = `${apiResponse.total_earnings}`
            })
        },
        earnings365() {
            this.axios.get("network/earnings/90d").then((response) => {
                const apiResponse = response.data
                this.earnings90d = `${this.floorFigure(apiResponse.total_earnings)}`
            })
        },
        earningspertask() {
            this.axios.get("network/provider/average/earnings").then((response) => {
                const apiResponse = response.data
