<template>
  <div>
    <b-card class="h-100">
      ...
      >
      <b-form-input class="mb-1 mt-1" type="number" :placeholder="this.count" @keyup.enter="fetchData($event.target.value)" />
      <b-table
        responsive
        hover
        outlined
...
<script>
import { BCard, BCol, BFormInput, BRow, BTable, BAvatar, BSpinner, BBadge } from "bootstrap-vue"
...
export default {
  ...
  methods: {
    ...
    fetchData(numberofnodes) {
      ...
      this.items.length = 0
      axios.get(`/v1/network/historical/nodes/${numberofnodes}`).then((response) => {
