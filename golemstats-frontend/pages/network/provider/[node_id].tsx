...
const useIncome = (node_id: string | undefined, initialIncome: object) => {
  const { data, error } = useSWR(
    node_id ? `v1/provider/node/${node_id}/earnings` : null,
    fetcher,
    {
...
const EarningSection = ({
  ...
  <div className="relative bg-white dark:bg-gray-800 pt-5 px-4 sm:pt-6 sm:px-6 shadow rounded-lg overflow-hidden">
    <dt>
      ...
      <p className="ml-16 text-sm font-medium text-gray-500 truncate dark:text-gray-400">
        {title}
      </p>
    </dt>
...
export const ProviderDetailed = ({
  initialData,
  initialIncome,
}: {
  initialData: object;
  initialIncome: object;
}) => {
  ...
  const { data: nodeData = initialData, error: nodeError } = useSWR(
    node_id ? `v1/provider/node/${node_id}` : null,
    fetcher,
    {
  ...
  const { income: updatedIncome, error: incomeError } = useIncome(
    node_id,
    initialIncome
  );
    ...
    <div className="min-h-full z-10 relative">
      <main className="pb-10">
          ...
          <div className="bg-white dark:bg-gray-800 col-span-12 rounded-lg px-6 overflow-auto py-4 md:flex md:items-center md:justify-between md:space-x-5">
            ...
            <div className="mt-6 flex flex-col-reverse justify-stretch space-y-4 space-y-reverse sm:flex-row-reverse sm:justify-end sm:space-x-reverse sm:space-y-0 sm:space-x-3 md:mt-0 md:flex-row md:space-x-3">
              <button
                onClick={() => {
                  router.push(
                    `/network/providers/operator/${nodeData[0].data["wallet"]}`
                  );
                }}
              ...
              <button
                ...
                onClick={() => {
                  if (
                    ...
                    window.open(
                      `https://polygonscan.com/address/${nodeData[0].data["wallet"]}#tokentxns`,
                      "_blank"
                    );
                  }
                }}
              ...
              <button
                ...
                onClick={() => {
                  if (
                    ...
                    window.open(
                      `https://goerli.etherscan.io/address/${nodeData[0].data["wallet"]}`,
                      "_blank"
                    );
                  } else {
                    window.open(
                      `https://etherscan.io/address/${nodeData[0].data["wallet"]}`,
                      "_blank"
                    );
                  }
                }}
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-flow-col-dense  lg:grid-cols-12">
            ...
            <dl className="mt-2 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="relative bg-white dark:bg-gray-800 pt-5 px-4 sm:pt-6 sm:px-6 shadow rounded-lg overflow-hidden">
                ...
                <dd className="ml-16 pb-6 flex items-baseline sm:pb-7">
                  <p className="text-2xl font-semibold text-gray-900 dark:text-gray-300">
                    {RoundingFunction(
                      nodeData[0].data["golem.inf.storage.gib"],
                      2
                    )}
                  </p>
                  <p className="text-golemblue ml-2 flex items-baseline text-sm font-semibold dark:text-gray-400">
          ...
          <section
            aria-labelledby="Node Pricing"
            className="lg:col-start-1 lg:col-span-6 col-span-12"
          >
            <h2
              id="Node Pricing"
          ...
          <section
            aria-labelledby="Node Earnings"
            className="lg:col-start-1 lg:col-span-12 col-span-12"
          >
            <h2
              id="Node Earnings"
              className="text-xl font-medium text-gray-900 dark:text-white"
