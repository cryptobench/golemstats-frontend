...
const FilterRow = ({ allKeys, onApply, data }) => {
  ...
  const allValuesForKey = Array.from(allValuesByKey.get(key) || []);

  const isKeyBoolean = useMemo(() => {
    return (
      allValuesForKey.length === 2 &&
      allValuesForKey.includes(true) &&
      allValuesForKey.includes(false)
    );
  }, [key, allValuesForKey]);
...
const Filters = ({ allKeys, onFilter, data }) => {
  ...
  const handleApply = (filter) => {
    ...
    const filterFunc =
      operator === "eq" && typeof value === "boolean"
        ? (dataValue) => dataValue === value
        : createFilterFunc(operator, value);

    setFilters((currentFilters) => {
      const updatedFilters = [
        ...currentFilters,
        { key, operator: operator, value: value, filterFunc },
      ];
      onFilter(
        updatedFilters.map(({ key, filterFunc }) => ({ key, filterFunc }))
      );
      ...
      <div className="filters">
        ...
        {filters.map((filter, index) => (
          <div key={index} className="flex items-center gap-x-4 mb-4">
            ...
            <span className="font-medium text-golemblue dark:text-white">
              {filter.operator}
            </span>
            <span className="font-semibold dark:text-gray-300">
              {filter.value}
...
export const ProviderList = ({ endpoint }) => {
  ...
  const {
    ...
    lastPage,
    setPage,
  } = useProviderPagination(filteredData);
            ...
            >
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <div className="text-sm text-gray-500 golemtext">
                      {provider.data["golem.node.debug.subnet"]}
                    </div>
                    {provider.data[
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <dt className="flex flex-row items-center">
                  ...
                  <p className="ml-2 text-sm font-medium text-gray-900 golemtext dark:text-gray-300">
                    {RoundingFunction(
                      provider.data["golem.inf.storage.gib"],
                      2
                    )}{" "}
                    GB
                  </p>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <a className="font-semibold text-gray-900 text-sm golemtext dark:text-gray-300">
                  ...
                  <span className="text-golemblue golemgradient dark:text-gray-400">
                    GLM
                  </span>
                </a>
              </td>
      ...
      <div className="flex justify-center mt-4 gap-2">
        <button
          ...
          className={`px-5 py-2 rounded-md ${
            page === 1
              ? "text-gray-400 bg-gray-200 dark:bg-golemblue/20 cursor-not-allowed"
              : "text-white bg-golemblue"
          }`}
        >
          Previous
        </button>
        {visiblePages.map((pageNumber) => (
          <button
            ...
            className={`px-5 py-2 rounded-md ${
              page === pageNumber
                ? "text-white bg-golemblue"
                : "text-white bg-golemblue/60 hover:bg-golemblue"
            }`}
          >
            {pageNumber}
          </button>
