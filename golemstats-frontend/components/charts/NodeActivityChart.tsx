...
const NodeActivityChart = ({ nodeId }: { nodeId: string }) => {
    const { data, error } = useSWR(`provider/node/${nodeId}/activity`, fetcher, { refreshInterval: 10000 })
    ...
    useEffect(() => {
        if (data && !error) {
            try {
                ...
                const computing = values.map(([time, value]: [number, number]) => [time * 1000, value])
    ...
    return (
        <section aria-labelledby="applicant-information-title">
            ...
            <div className="bg-white dark:bg-gray-800 mt-2 pt-5 px-4 sm:py-6 sm:px-6 shadow rounded-lg">
                <h1 className="text-xl font-medium dark:text-gray-300">Tasks being computed</h1>
                <p className="text-sm text-gray-500 dark:text-gray-400">1 = Computing / 0 = Inactive</p>
